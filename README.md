...
**Resultado medido, com a limitação explícita.** Na primeira execução completa (`make ml`, 2026-08-25), o `ML.EVALUATE` sobre o split de avaliação reservado deu **ROC AUC 0,570**, com recall 1,0 e precisão 0,516 — ou seja, o modelo marca praticamente todo município como "vai atingir a meta", e o log-loss de 0,6925 é o de um classificador que responde 50% para qualquer caso (`ln 2 ≈ 0,6931`). A view `ml_predicao_risco_municipio` pontua 10.691 municípios, então o caminho ponta a ponta — treino, avaliação e predição servida — está fechado e auditável; o que ainda não existe é poder discriminativo. A causa provável está no conjunto de features: apenas `ano` + IDHM e suas componentes, sobre um rótulo que só existe em 2024 (em 2023 `atingiu_meta` é NULL porque não há meta municipal vigente) e com as classes quase empatadas (2.783 municípios atingiram a meta contra 2.444 que não atingiram). Enriquecer as features com Censo Escolar, FUNDEB e a série histórica de proficiência é o próximo passo, e é exatamente o item de fontes externas que segue aberto no [Roadmap](#roadmap).

### Análise de desigualdade educacional

Clusterização de municípios por perfil socioeducacional, combinando os indicadores de `fact_indicador_municipio`/`fact_alfabetizacao_municipio` com os atributos territoriais e de desenvolvimento humano de `dim_municipio` (região, capital, IDHM e suas 3 componentes). Isso permite agrupar municípios comparáveis por contexto socioeconômico e medir o efeito real de uma política controlando por esse contexto — evitando comparar, por exemplo, um município de alta renda com um de baixo IDHM e atribuir toda a diferença de resultado à gestão local. O mesmo cruzamento serve para mapear desigualdade regional: `mart_ranking_indicador_municipio` já rankeia municípios dentro da própria UF, evidenciando dispersão entre pares na mesma unidade federativa em vez de só a média nacional.

### Políticas públicas baseadas em dados

Séries temporais em `fact_meta_resultado_{brasil,uf,municipio}` (`gap_pontos`, `atingiu_meta`, trajetória de metas por ano, versionada via SCD Tipo 2) permitem simular cenários — "o que aconteceria com a meta nacional se a UF X replicasse a trajetória da UF Y" — e priorizar investimento onde o retorno marginal em alfabetização é maior. `mart_evolucao_indicador_uf` e `mart_aderencia_metas_uf` já entregam essa leitura pronta (variação ano a ano e percentual de cumprimento de meta por UF), reduzindo a distância entre a camada analítica e a decisão de gestor público. Um banco vetorial sobre embeddings de perfil municipal (ver trade-off de NoSQL abaixo) é uma extensão natural desse caso de uso: consultas como "encontrar municípios com contexto parecido ao do município X" apoiam a transferência de políticas que funcionaram entre gestões locais comparáveis.
